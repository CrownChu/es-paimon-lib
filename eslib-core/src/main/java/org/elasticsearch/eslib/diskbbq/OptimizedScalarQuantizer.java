...
public class OptimizedScalarQuantizer {
    ...
    public static final class QuantizationResult {
        ...
        public float lowerInterval() { return lowerInterval; }
        public float upperInterval() { return upperInterval; }
        public float additionalCorrection() { return additionalCorrection; }
        public int quantizedComponentSum() { return quantizedComponentSum; }
    }
    ...
    private boolean optimizeIntervals(float[] initInterval, int[] destination, float[] vector, float norm2, int points) {
        ...
        for (int i = 0; i < iters; ++i) {
            ...
            if (bOpt < aOpt) {
                // numerical instability produced an inverted interval (lower > upper); keep the previous valid one
                return true;
            }
