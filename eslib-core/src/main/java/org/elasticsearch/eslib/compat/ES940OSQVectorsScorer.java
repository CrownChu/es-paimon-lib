 ...
 *
 * <p>Pure-Java port of Elasticsearch's {@code org.elasticsearch.simdvec.ES940OSQVectorsScorer}.
 * The ES940 DiskBBQ writer only references {@link #BULK_SIZE}; the scoring methods are ported
 * verbatim for the (not-yet-wired) reader path. The single native {@code JdkFeatures.SUPPORTS_HEAP_SEGMENTS}
 * SIMD branch is replaced by its scalar fallback (byte-identical result, reader-side only).
 */
public class ES940OSQVectorsScorer {
    ...
    private long quantized4BitScorePacked(byte[] q) throws IOException {
        ...
        // Native uses VectorUtil.int4DotProductSinglePacked when heap MemorySegments are supported;
        // the scalar fallback below is byte-identical and avoids the JdkFeatures dependency.
        long score = 0;
