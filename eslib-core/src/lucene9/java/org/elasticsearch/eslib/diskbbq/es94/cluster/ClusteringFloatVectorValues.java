...
package org.elasticsearch.eslib.diskbbq.es94.cluster;
...
/**
 * JDK 11 / Lucene 9.12 port: was {@code abstract sealed class ... extends FloatVectorValues
 * permits KMeansFloatVectorValues, ClusteringFloatVectorValuesSlice}. Lucene 9.12's
 * {@code FloatVectorValues} is a cursor (extends DocIdSetIterator) with no random-access
 * {@code vectorValue(int)}, so we make this a standalone random-access abstraction (mirroring
 * the lucene9 {@code IndexedFloatVectorValues} shim). The writer only uses the random-access
 * methods declared here, so the produced bytes are unchanged.
 */
public abstract class ClusteringFloatVectorValues {

    /** Random-access vector by ordinal. */
    ...
    /** Map an ordinal to its doc id; identity by default. */
    ...
    // JDK 11 port: was `private record PrefixScratch(float[] topPrefixDistances, int[] topPrefixIds,
    // int prefixLength)`. Hand-expanded to a final class with the same fields/accessors; no behavior change.
    private static final class PrefixScratch {
        private final float[] topPrefixDistances;
        private final int[] topPrefixIds;
        private final int prefixLength;

        PrefixScratch(float[] topPrefixDistances, int[] topPrefixIds, int prefixLength) {
            this.topPrefixDistances = topPrefixDistances;
            this.topPrefixIds = topPrefixIds;
