...
import org.elasticsearch.eslib.compat.ES940OSQVectorsScorer;
...
import static org.elasticsearch.eslib.compat.ES940OSQVectorsScorer.BULK_SIZE;
import static org.elasticsearch.eslib.diskbbq.OptimizedScalarQuantizer.DEFAULT_LAMBDA;
import static org.elasticsearch.eslib.diskbbq.es94.PostingMetadata.NO_ORDINAL;

/**
 * Standalone-lib (Lucene 9.12 / JDK 11) ES940 DiskBBQ reader. Port of the native
 * {@code org.elasticsearch.index.codec.vectors.diskbbq.es94.ES940DiskBBQVectorsReader} onto the
 * simplified {@link IVFVectorsReader} base. The ES940 algorithm (two-level parent centroids +
 * asymmetric query quantization) is preserved byte-for-byte against what
 * {@link ES940DiskBBQVectorsWriter} writes.
 *
 * <p>Standalone deltas vs the mount reader: Lucene 10 {@code AcceptDocs} becomes {@link Bits} and
 * accept-docs filtering is applied per-doc in the posting visitor (no centroid pre-filtering); OSS
 * bulk-prefetch, per-cluster exact-read extents, and the per-query profiler are dropped.
 */
public class ES940DiskBBQVectorsReader extends IVFVectorsReader implements VectorPreconditioner {

    ES940DiskBBQVectorsReader(SegmentReadState state, IVFVectorsReader.LoadFlatVectorsReader getFormatReader) throws IOException {
    ...
    public CentroidIterator getCentroidIterator(
        ...
        // Standalone lib: no cheap accept-docs iterator/cost is available from a Bits, so we never
        // pre-filter centroids. Accept-docs filtering is applied per-doc in the posting visitor.
        final NextFieldEntry fieldEntry = (NextFieldEntry) fields.get(fieldInfo.number);
        int bulkSize = fieldEntry.getBulkSize();
        final int bitsRequired = DirectWriter.bitsRequired(numCentroids);
        final long sizeLookup = directWriterBytesRequired(values.size(), bitsRequired);
        ...
        final ES92Int7VectorsScorer scorer = new ES92Int7VectorsScorer(centroids, fieldInfo.getVectorDimension(), bulkSize);
    ...
    // JDK 11 / Lucene 9.12: DirectWriter.bytesRequired(numValues, bitsPerValue) does not exist in 9.12.
    // Mirror of the writer's directWriterBytesRequired so the centroid doc->ordinal lookup table is
    // skipped by exactly the number of bytes DirectWriter.finish() wrote.
    private static long directWriterBytesRequired(long numValues, int bitsPerValue) {
    ...
    @Override
    protected IVFVectorsReader.FieldEntry doReadField(
        IndexInput input,
    ...
    public Preconditioner getPreconditioner(FieldInfo fieldInfo) throws IOException {
        ...
        if (preconditionerLength > 0) {
            try (IndexInput ivfPreconditionerSlice =
                ivfCentroids.slice(
                    "preconditioner", preconditionerOffset, preconditionerLength
                )) {
    ...
    static class NextFieldEntry extends IVFVectorsReader.FieldEntry {
    ...
    public PostingVisitor getPostingVisitor(
        ...
        NextFieldEntry entry = (NextFieldEntry) fields.get(fieldInfo.number);
        final int bitsRequired = DirectWriter.bitsRequired(entry.numCentroids());
        final long sizeLookup = directWriterBytesRequired(numVectors, bitsRequired);
        ...
        return new MemorySegmentPostingsVisitor(queryQuantizer, quantEncoding, indexInput, entry, fieldInfo, acceptDocs, numVectors);
    }

    private static final class QueryQuantizerResult {
        private final OptimizedScalarQuantizer.QuantizationResult queryCorrections;
        private final byte[] quantizedTarget;

        QueryQuantizerResult(OptimizedScalarQuantizer.QuantizationResult queryCorrections, byte[] quantizedTarget) {
            this.queryCorrections = queryCorrections;
            this.quantizedTarget = quantizedTarget;
        }
    ...
    private static class QueryQuantizer {
        ...
        QueryQuantizer(
            ...
            this.cache = new LinkedHashMap<Integer, QueryQuantizerResult>(QUERY_CACHE_SIZE, 0.75f, true) {
                @Override
        ...
        void quantizeQueryIfNecessary() throws IOException {
            if (nextCentroidOrdinal != currentCentroidOrdinal) {
                QueryQuantizerResult quantized = cache.get(nextCentroidOrdinal);
                if (quantized != null) {
                    result = quantized;
    ...
    private static class MemorySegmentPostingsVisitor implements PostingVisitor {
        ...
        MemorySegmentPostingsVisitor(
            ...
            osqVectorsScorer = new ES940OSQVectorsScorer(
                indexInput,
                quantEncoding.queryBits(),
                quantEncoding.bits(),
        ...
        public int resetPostingsScorer(PostingMetadata metadata) throws IOException {
            ...
            // The score is the transformed score used when searching the centroids; convert it back to
            // the raw similarity to be used as part of the final corrections.
            switch (similarityFunction) {
                case EUCLIDEAN:
                    centroidDistance = ((1 / score) - 1) - centroidToParentSqDist;
                    break;
                case COSINE:
                case DOT_PRODUCT:
                    centroidDistance = 2 * score - 1;
                    break;
                case MAXIMUM_INNER_PRODUCT:
                    centroidDistance = score - 1;
                    break;
                default:
                    throw new IllegalStateException("Unsupported similarity function: " + similarityFunction);
            }
        ...
        private void collectBulk(KnnCollector knnCollector, float[] scores, int bulkSize) {
            for (int i = 0; i < bulkSize; i++) {
                ...
                // doc == -1 means filtered out by acceptDocs. SOAR duplicates are handled by the
                // search-level deduplicating collector so later higher-scored replicas can still win.
                if (doc != -1) {
