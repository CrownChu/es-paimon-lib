...
// JDK 11 port: was `public record NeighborHood(int[] neighbors, float maxIntraDistance)`.
// Hand-expanded to a final class with the same component accessors; no behavior/byte change.
public final class NeighborHood {
    ...
    private static void populateNeighboursFromGraph(
        ...
        for (int i = start; i < end; i++) {
            // JDK 11 / Lucene 9.12 port: 9.12's RandomVectorScorerSupplier exposes scorer(int) returning
            // a fresh RandomVectorScorer (no UpdateableRandomVectorScorer / setScoringOrdinal). Same search.
    ...
    // JDK 11 / Lucene 9.12 port: was `private record CentersScorerSupplier(float[][] centers,
    // UpdateableRandomVectorScorer scorer)`. Lucene 9.12 has no UpdateableRandomVectorScorer and the
    // supplier exposes scorer(int) returning a fresh RandomVectorScorer (no bulkScore/setScoringOrdinal).
    // The score is normalizeDistanceToUnitInterval(d2) == 1/(1+d2); populateNeighboursFromGraph inverts
    // it via (1f/minScore)-1, so the resulting neighborhoods/maxIntraDistance are unchanged.
        ...
        public RandomVectorScorer scorer(int ord) {
            ...
            return new RandomVectorScorer() {
                ...
                public float score(int node) {
                    float d2 = ESVectorUtil.squareDistance(centers[scoringOrdinal], centers[node]);
                    return 1.0f / (1.0f + d2);
                }
