...
public class HierarchicalKMeans {
     ...
     */
    public KMeansResult cluster(IndexedFloatVectorValues vectors, int targetSize) throws IOException {
        if (vectors.size() == 0) {
    ...
    private KMeansIntermediate clusterAndSplit(final IndexedFloatVectorValues vectors, final int targetSize) throws IOException {
        if (vectors.size() <= targetSize) {
            return new KMeansIntermediate();
        }

        // JDK 11 port: Math.clamp is JDK 21+. Inline clamp(value, 2, MAXK).
        int k = Math.max(2, Math.min((int) ((vectors.size() + targetSize / 2.0f) / (float) targetSize), MAXK));
        ...
        for (int c = 0; c < centroidVectorCount.length; c++) {
            ...
            if (100 * count > 134 * targetSize) {
                final IndexedFloatVectorValues sample = createClusterSlice(count, adjustedCentroid, vectors, assignments);
    ...
    static IndexedFloatVectorValues createClusterSlice(int clusterSize, int cluster, IndexedFloatVectorValues vectors, int[] assignments) {
