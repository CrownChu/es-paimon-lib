...
class FloatVectorValuesSlice extends IndexedFloatVectorValues {

    private final IndexedFloatVectorValues allValues;
    private final int[] slice;

    FloatVectorValuesSlice(IndexedFloatVectorValues allValues, int[] slice) {
        assert slice != null;
    ...
    @Override
    public IndexedFloatVectorValues copy() throws IOException {
        return new FloatVectorValuesSlice(this.allValues.copy(), this.slice);
    }

    static IndexedFloatVectorValues createRandomSlice(IndexedFloatVectorValues origin, int k, long seed) {
        if (k >= origin.size()) {
            return origin;
        }
