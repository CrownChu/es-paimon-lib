...
public final class NeighborHood {
    ...
    private static final class CentersScorerSupplier implements RandomVectorScorerSupplier {
        ...
        CentersScorerSupplier(float[][] centers, UpdateableRandomVectorScorer scorer) {
        ...
        CentersScorerSupplier(float[][] centers) {
            this(centers, new UpdateableRandomVectorScorer() {
                ...
                public float score(int node) {
                    return org.elasticsearch.eslib.diskbbq.VectorCompat.normalizeDistanceToUnitInterval(VectorUtil.squareDistance(centers[scoringOrdinal], centers[node]));
                }
                ...
                public float bulkScore(int[] nodes, float[] scores, int numNodes) {
                    ...
                    for (; i < limit; i += 4) {
                        ...
                        for (int j = 0; j < 4; j++) {
                            scores[i + j] = org.elasticsearch.eslib.diskbbq.VectorCompat.normalizeDistanceToUnitInterval(distances[j]);
        ...
        public float[][] centers() { return centers; }
    }
