...
public class ES920DiskBBQVectorsReader extends IVFVectorsReader {
    ...
    // (and the package was relocated in Lucene 10.4).
    private static final byte QUERY_BITS = 4;

    ES920DiskBBQVectorsReader(SegmentReadState state, IVFVectorsReader.LoadFlatVectorsReader getFormatReader) throws IOException {
    ...
    public CentroidIterator getPostingListPrefetchIterator(CentroidIterator centroidIterator, IndexInput postingListSlice)
        throws IOException {
        // Drain the centroid iterator once so we can fan out a bulk prefetch ahead of the serial
        // scoring loop. For cohere-sized tables there are typically 5-15 candidate clusters per
        // query (visit_ratio × num_centroids); draining is cheap (it's already doing the
        // centroid scoring work serially otherwise). The 8.17 fork serializes cluster reads
        // with a 1-ahead prefetch — that overlaps network of cluster N+1 with CPU of cluster N
        // but caps total in-flight at 2. Fanning out via prefetchBulk lets up to PREFETCH_POOL
        // (8-16 thread) reads run truly in parallel; the scorer still consumes serially below
        // but only ever waits for bytes it didn't see fully prefetched.
        final java.util.List<CentroidOffsetAndLength> all = new java.util.ArrayList<>();
        while (centroidIterator.hasNext()) {
            all.add(centroidIterator.nextPostingListOffsetAndLength());
        }
        if (all.isEmpty() == false) {
            if (postingListSlice instanceof org.elasticsearch.eslib.io.OSSRangeIndexInput oss) {
                long[] offs = new long[all.size()];
                int[] lens = new int[all.size()];
                for (int i = 0; i < all.size(); i++) {
                    offs[i] = all.get(i).offset();
                    lens[i] = (int) Math.min(all.get(i).length(), Integer.MAX_VALUE);
                }
                oss.prefetchBulk(offs, lens);
            } else {
                // MMapDirectory / ByteBuffersDirectory etc. — fall back to per-cluster Lucene
                // prefetch hint. Single-slot in our OSS case but mostly a no-op on local IO.
                for (CentroidOffsetAndLength c : all) {
                    postingListSlice.prefetch(c.offset(), c.length());
                }
            }
        }
        final java.util.Iterator<CentroidOffsetAndLength> it = all.iterator();
